...
d_matilda <- d_data %>%
  ...
  select(
    Instances,
    starts_with("f_"),-f_vcg_graph,-f_vg_graph,
  ...
  filter(!is.na(algo_time_T_100_tstep_1),!is.na(algo_time_T_400_tstep_1)) %>%
...
d_res <- d_matilda %>%
  filter(algo_time_T_200_tstep_1 < 0.5,
...
scale_cols <-
  c(
    "feature_p_size_n_variables",
    "feature_vcg_mean_cls",
    "feature_vcg_min_cls",
    "feature_vcg_max_cls"
  )
