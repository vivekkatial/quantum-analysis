load_instance_exact_cover = function(run_id, experiment_name, uri = "http://115.146.95.176:5000/") {
...
load_instance_exact_cover = function(run_id, experiment_name, uri = "http://115.146.95.176:5000/") {
  ...
  browser()
  # Download Artifacts
  mlflow::mlflow_download_artifacts("tmp/")
  # Get clauses
  d_results <- read_rds("tmp/d_clauses.rds")
  # Delete temp file
  file.remove("tmp/d_clauses.rds")
  # Get number of qubits
  n_qubits <- mlflow::mlflow_get_run(run_id = run_id) %>%
